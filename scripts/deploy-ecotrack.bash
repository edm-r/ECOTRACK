...
ensure_docker_access() {
  local attempts="$DOCKER_START_TIMEOUT"
...
clone_project() {
  ...
  local clone_args=("$REPO_URL" "$PROJECT_DIR")
  if [[ -n "$BRANCH" ]]; then
    clone_args=(--branch "$BRANCH" --single-branch "$REPO_URL" "$PROJECT_DIR")
  fi
